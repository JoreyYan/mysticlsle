-- ChillFit Rave 真实产品数据插入脚本（最终修复版）
-- 修复了SKU和变体name字段的问题
...
-- 为套装产品添加颜色变体 - 包含name字段
INSERT INTO product_variants (product_id, name, sku, price, sale_price, stock_quantity, attributes)
SELECT
    p.id,
    p.name || ' - ' || color_info.color_name,  -- 添加了name字段
...
SELECT COUNT(*) as "总产品数" FROM products WHERE sku LIKE 'CFR-%';
SELECT COUNT(*) as "总图片数" FROM product_images pi
JOIN products p ON pi.product_id = p.id
WHERE p.sku LIKE 'CFR-%';
SELECT COUNT(*) as "总变体数" FROM product_variants pv
